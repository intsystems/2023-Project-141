...
Для простой неросети $y=\sigma(Wx)$ мы иссоедуем строки матрицы W, как линейные функционалы в сопряженном пространстве. Каждая такая строка имеет матожидание и матрицу ковариации. Строки и являются объектом исследования.
...
**Ожидаемый результат:** состоит в том, что пространство параметров-строк будет простой структуры, будет обладать структурой сообществ, которая позволит для каждого нейрона определить, насколько он значим (по матрице ковариации), отобрать параметры или преобразовать их.
